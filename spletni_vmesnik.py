...
def ustvari_novo_mapo_za_program(uporabnik, ime_stopnje, ime_programa):
    pot = os.path.join(
        f"./views/datoteke/{uporabnik.uporabnisko_ime}/{ime_stopnje}", ime_programa)
    os.mkdir(pot)
...
def ustvari_novo_mapo_za_vajo(uporabnik, ime_stopnje, ime_programa, ime_vaje):
    pot = os.path.join(
        f"./views/datoteke/{uporabnik.uporabnisko_ime}/{ime_stopnje}/{ime_programa}", ime_vaje)
    if os.path.isdir(pot):
        pass
    else:
        os.mkdir(pot)
...
def osnovna_stran():
    ...
    return bottle.template(
        "osnovna_stran.html", zvezek=uporabnik.zvezek, uporabnik=uporabnik, napaka = "Stopnja s tem imenom že obstaja!")
...
def ogled_stopnje(ime_stopnje):
    ...
    return bottle.template("ogled_stopnje.html", stopnja=iskana_stopnja,
                           ...
                           napaka = "Program s tem imenom na tej stopnji že obstaja!")
...
def ogled_programa(ime_stopnje, ime_programa):
    ...
    iskana_stopnja = trenutna_stopnja(uporabnik, ime_stopnje)
    iskan_program = trenutni_program(uporabnik, ime_programa, ime_stopnje)
    return bottle.template("ogled_programa.html", uporabnik=uporabnik,  stopnja=iskana_stopnja, program=iskan_program,
                            vaje=uporabnik.zvezek.vaje_v_programu(iskan_program),
                            ...
                            napaka = "Vaja s tem imenom v tem programu že obstaja!")
...
def dodaj_vajo(ime_stopnje, ime_programa):
    ...
    uporabnik.zvezek.dodaj_vajo(
        ime_vaje, iskan_program, kategorija, opis, ime_glasbe, ime_posnetka)
    shrani_stanje(uporabnik)
...
def uredi_vajo_post(ime_stopnje, ime_programa, ime_vaje):
    ...
    shrani_stanje(uporabnik)
    bottle.redirect(
        f"/stopnja_{ime_stopnje}/program_{ime_programa}/vaja_{ime_vaje}/")
